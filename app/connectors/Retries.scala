...
trait Retries {
  ...
  val shouldRetry: Try[HttpResponse] => Boolean =
    _.fold(NonFatal(_), response => response.status == 469 || response.status >= 499)

  val retryReason: HttpResponse => String =
    response => s"received $response"

  final def retry[A](intervals: FiniteDuration*)(shouldRetry: Try[A] => Boolean, retryReason: A => String)(
    block: => Future[A]
