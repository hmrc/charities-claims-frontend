...
class OtherIncomeScheduleUploadStatusController @Inject() (
      ...
      .async { implicit request =>
        {
          for {
            ...
            _                   <- request.sessionData.repaymentClaimDetailsAnswers
                                     .flatMap(_.claimingTaxDeducted)
                                     .flatMap(Option.when(_)(()))
            fileUploadReference <- request.sessionData.otherIncomeScheduleFileUploadReference
