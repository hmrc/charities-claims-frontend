...
class CommunityBuildingsScheduleUploadStatusController @Inject() (
      ...
      .async { implicit request =>
        {
          for {
            ...
            _                   <- request.sessionData.repaymentClaimDetailsAnswers
                                     .flatMap(_.claimingDonationsCollectedInCommunityBuildings)
                                     .flatMap(Option.when(_)(()))
            fileUploadReference <- request.sessionData.communityBuildingsScheduleFileUploadReference
