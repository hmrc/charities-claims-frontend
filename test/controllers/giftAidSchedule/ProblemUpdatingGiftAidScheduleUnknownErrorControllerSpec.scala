...
class ProblemUpdatingGiftAidScheduleUnknownErrorControllerSpec extends ControllerSpec {

  "ProblemUpdatingGiftAidScheduleUnknownErrorController" - {
    "onPageLoad" - {
      "should render the page correctly" in {
        ...
        running(application) {
          val request: FakeRequest[AnyContentAsEmpty.type] =
            FakeRequest(GET, routes.ProblemUpdatingGiftAidScheduleUnknownErrorController.onPageLoad.url)
      ...
      "should decline when schedule checkbox is not selected" in {
        ...
        running(application) {
          val request = FakeRequest(GET, routes.ProblemUpdatingGiftAidScheduleUnknownErrorController.onPageLoad.url)
          val result  = route(application, request).value
    ...
    "onSubmit" - {
      "should redirect to the next page" in {
        ...
        running(application) {
          val request = FakeRequest(POST, routes.ProblemUpdatingGiftAidScheduleUnknownErrorController.onSubmit.url)
          val result  = route(application, request).value
          status(result)           shouldBe SEE_OTHER
