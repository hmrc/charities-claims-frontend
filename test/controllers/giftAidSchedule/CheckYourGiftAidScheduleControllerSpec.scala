...
class CheckYourGiftAidScheduleControllerSpec extends ControllerSpec {
  ...
  // test session data that passes DataGuard (shouldUploadGiftAidSchedule) using completeGasdsSession as base
  ...
  def validSessionDataCompleted: SessionData = validSessionData.copy(giftAidScheduleCompleted = true)

  def sessionDataFailingGuard: SessionData = RepaymentClaimDetailsAnswers
    .setClaimingUnderGiftAidSmallDonationsScheme(true)
    .and(RepaymentClaimDetailsAnswers.setClaimingGiftAid(false))
    .and(SessionData.setUnsubmittedClaimId(testClaimId))

  "CheckYourGiftAidScheduleController" - {
    ...
    "onPageLoad" - {
      "should render ClaimCompleteController if submissionReference is defined" in {
        ...
        running(application) {
          given request: FakeRequest[AnyContentAsEmpty.type] =
            FakeRequest(GET, routes.CheckYourGiftAidScheduleController.onPageLoad.url)
      ...
      "should render the page when schedule data is available" in {
        ...
        running(application) {
          ...
          contentAsString(result) should include("Check your Gift Aid schedule")
        }
      }

      "should display gift Aid data in the table" in {
        (mockClaimsValidationService
          .getGiftAidScheduleData(using _: DataRequest[?], _: HeaderCarrier))
          .expects(*, *)
      ...
      "should default to page 1 when page query parameter is not a valid integer" in {
        ...
        running(application) {
          given request: FakeRequest[AnyContentAsEmpty.type] =
            FakeRequest(GET, routes.CheckYourGiftAidScheduleController.onPageLoad.url + "?page=invalid")

          val result  = route(application, request).value
          val content = contentAsString(result)

          status(result) shouldEqual OK
          // the first gift aid from test json file should be visible on page 1
          content should include("Henry")
        }
      }
    }
    ...
    "onSubmit" - {
      "should render ClaimCompleteController if submissionReference is defined" in {
        ...
        running(application) {
          given request: FakeRequest[AnyContentAsEmpty.type] =
            FakeRequest(GET, routes.CheckYourGiftAidScheduleController.onSubmit.url)
      ...
      "should return BadRequest when form onSubmit has errors and display error message" in {
        ...
        running(application) {
          given request: FakeRequest[AnyContentAsFormUrlEncoded] =
            FakeRequest(POST, routes.CheckYourGiftAidScheduleController.onSubmit.url)
          ...
          content should include("Check your Gift Aid schedule")
          content should include("Select ‘Yes’ if you need to update this Gift Aid schedule")
        }
      }
      ...
      "should redirect to update screen when Yes is selected" in {
        ...
        running(application) {
          ...
          status(result) shouldEqual SEE_OTHER
          redirectLocation(result) shouldEqual Some(
            routes.UpdateGiftAidScheduleController.onPageLoad.url
          )
        }
      }
      ...
      "should save and redirect to UploadSuccessful when No is selected and schedule not yet completed before" in {
        ...
        running(application) {
          ...
          status(result) shouldEqual SEE_OTHER
          redirectLocation(result) shouldEqual Some(
            routes.GiftAidScheduleUploadSuccessfulController.onPageLoad.url
          )
        }
      }
    }
