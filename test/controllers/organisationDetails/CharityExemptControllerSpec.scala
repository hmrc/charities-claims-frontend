  ...
  "CharityExemptController" - {
      ...
      "should render the page correctly" in {
        ...
        running(application) {
          given request: FakeRequest[AnyContentAsEmpty.type] =
            FakeRequest(GET, routes.CharityExemptController.onPageLoad.url)
    ...
    "onSubmit" - {
      "should redirect to the next page" in {
        ...
        running(application) {
          given request: FakeRequest[AnyContentAsEmpty.type] =
            FakeRequest(POST, routes.CharityExemptController.onSubmit.url)

          val result = route(application, request).value

          status(result) shouldEqual SEE_OTHER
          redirectLocation(result) shouldEqual Some(routes.CharityExemptController.onPageLoad.url)
        }
      }
    }
  }
