    ...
    "onPageLoad" - {
      "should render the page correctly" in {
        ...
        running(application) {
          ...
          contentAsString(result) should include("You cannot manage this claim")
        }
      }

      "should use the correct configured makeCharityRepaymentClaimUrl in the message" in {
        val customConfig = Map(
          "urls.makeCharityRepaymentClaimUrl" -> "https://test.example.com/charity-repayment-claim"
        )
        ...
        running(application) {
          val request = FakeRequest(GET, routes.CannotViewOrManageClaimController.onPageLoad.url)
          val result  = route(application, request).value
