...
class ProblemUpdatingCommunityBuildingsScheduleQuarantineControllerSpec extends ControllerSpec {
  ...
  "ProblemUpdatingCommunityBuildingsScheduleQuarantineController" - {
    "onPageLoad" - {
    ...
    "onSubmit" - {
      "should render ClaimCompleteController if submissionReference is defined" in {
        ...
        running(application) {
          given request: FakeRequest[AnyContentAsEmpty.type] =
            FakeRequest(GET, routes.ProblemUpdatingCommunityBuildingsScheduleQuarantineController.onSubmit.url)
      ...
      "should redirect to the next page" in {
        ...
        running(application) {
          val request =
            FakeRequest(POST, routes.ProblemUpdatingCommunityBuildingsScheduleQuarantineController.onSubmit.url)
          val result  = route(application, request).value
          status(result)           shouldBe SEE_OTHER
