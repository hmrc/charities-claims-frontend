...
class ProblemUpdatingOtherIncomeScheduleRejectedControllerSpec extends ControllerSpec {

  "ProblemUpdatingOtherIncomeScheduleRejectedController" - {
    "onPageLoad" - {
      "should render the page correctly" in {
        ...
        running(application) {
          val request: FakeRequest[AnyContentAsEmpty.type] =
            FakeRequest(GET, routes.ProblemUpdatingOtherIncomeScheduleRejectedController.onPageLoad.url)
      ...
      "should decline when schedule checkbox is not selected" in {
        ...
        running(application) {
          val request = FakeRequest(GET, routes.ProblemUpdatingOtherIncomeScheduleRejectedController.onPageLoad.url)
          val result  = route(application, request).value
    ...
    "onSubmit" - {
      "should redirect to the next page" in {
        ...
        running(application) {
          val request = FakeRequest(POST, routes.ProblemUpdatingOtherIncomeScheduleRejectedController.onSubmit.url)
          val result  = route(application, request).value
          status(result)           shouldBe SEE_OTHER
