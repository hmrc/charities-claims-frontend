...
class UploadCommunityBuildingsScheduleControllerISpec
  ...
  private val url = "/upload-community-buildings-schedule"

  "GET /upload-community-buildings-schedule" should {
    ...
    "render the upload page when upscan is already initialised" in {
      ...
      Jsoup.parse(result.body).title should include(msg("uploadCommunityBuildingsSchedule.title"))
    }
  ...
  "GET /upload-success" should {
    ...
    "update upload status and redirect to upload summary page" in {
      ...
      val result = get("/upload-community-buildings-schedule/success")
  ...
  "GET /upload-error" should {
    ...
    "render page with error code when upscan exists" in {
      ...
      val result = get("/upload-community-buildings-schedule/error?errorCode=EntityTooLarge")
    ...
    "redirect to upload page when upscan session missing" in {
      ...
      val result = get("/upload-community-buildings-schedule/error")
  ...
  private def stubBackend(
    ...
    val claimResponse =
      claim.copy(
        claimData = claim.claimData.copy(
          ...
          communityBuildingsScheduleFileUploadReference = if withReference then Some(communityBuildingsFileRef) else None
        )
      )
    ...
    if withReference then
      stubGetUploadResult(
        claimId,
        communityBuildingsFileRef
      )(OK, getUploadResultCommunityBuildingsValidatedJson)
