...
class GiftAidScheduleUploadStatusControllerISpec
    extends ComponentSpecHelper with TestDataUtils
    with ClaimsStub
      with AuthStub
      with ClaimsValidationStub {

  private val url     = "/gift-aid-schedule-upload-status"

  "GET /gift-aid-schedule-upload-status" should {
    ...
    "return BAD_REQUEST when backend returns CLAIM_REFERENCE_DOES_NOT_EXIST" in {
      ...
      stubGetUploadResult(claimId, giftAidFileRef)(BAD_REQUEST, Json.obj("message" -> "CLAIM_REFERENCE_DOES_NOT_EXIST"))
    ...
    "return INTERNAL_SERVER_ERROR when backend throws other error" in {
      ...
      stubGetUploadResult(claimId, giftAidFileRef)(INTERNAL_SERVER_ERROR, Json.obj())
