...
class UpdateGiftAidControllerISpec
  extends ComponentSpecHelper with TestDataUtils
    with ClaimsStub
    with AuthStub
    with ClaimsValidationStub {

  private val url = "/update-gift-aid-schedule"

  "GET /update-gift-aid-schedule" should {

    "render the update gift aid schedule page" in {
      stubBackend()
      ...
      doc.title should include(msg("updateGiftAidSchedule.title"))
    }
  }

  "POST /update-gift-aid-schedule" should {
    ...
    "delete schedule and redirect to upload page when answer is Yes" in {
      ...
      stubDeleteSchedule(claimId, giftAidFileRef)(OK, Json.toJson(deleteResponse))
    ...
    "redirect to check your gift aid schedule page when answer is No" in {
      stubBackend()
  ...
  private def stubBackend(): Unit = {
    ...
    stubGetClaims(
      claimId
    )(OK, Json.toJson(claimWithGiftAid))
