...
class CharityExemptControllerISpec
  extends ComponentSpecHelper with TestDataUtils
    with ClaimsStub
    with AuthStub
    with ClaimsValidationStub {

  private val normalUrl = "/charity-exempt"
  private val checkModeUrl = "/change-charity-exempt"

  "GET /charity-exempt" should {

    "render the page when reason not registered is Exempt" in {
      stubBackend(claimWithReason(Some(ReasonNotRegisteredWithRegulator.Exempt)))
      ...
      doc.title should include(msg("charityExempt.title"))
    }

    "redirect to task list when reason not registered is not Exempt" in {
