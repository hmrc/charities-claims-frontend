...
class CorporateTrusteeAddressControllerISpec extends ComponentSpecHelper
  with AuthStub with TestDataUtils with ClaimsStub with ClaimsValidationStub {

  private val normalUrl    = "/corporate-trustee-address"
  private val checkModeUrl = "/change-corporate-trustee-address"

  "GET /corporate-trustee-address" should {

    "render the page when user is a corporate trustee" in {
      stubBackend()
      ...
      doc.title should include(msg("corporateTrusteeAddress.title"))
    }

    "redirect to task list when user is not a corporate trustee" in {
  ...
  "POST /corporate-trustee-address" should {
    ...
    "redirect to corporate trustee details in NormalMode" in {
      stubBackend()
      ...
      result.status               shouldBe SEE_OTHER
      result.header(LOCATION).value should include("/corporate-trustee-details")
    }
